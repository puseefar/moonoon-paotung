...
const PRO_MOODS: Record<MoodKey, { label: string; dot: string; emoji: string }> = {
  ...
  ok:    { label: 'เฉยๆ',   dot: '#AFC5D8', emoji: '😌' },
  tired: { label: 'เหนื่อย', dot: '#B9A8C9', emoji: '😤' },
  bad:   { label: 'แย่',    dot: '#C9A0A8', emoji: '😢' },
};
...
export default function DiarySearchScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.paper }}>
      ...
      {/* ── Sticky top ───────────────────────────────────────────────────────── */}
      <View style={{ paddingTop: insets.top + 8, backgroundColor: C.paper,
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, marginBottom: 10 }}>
          <Pressable onPress={() => router.back()}
            style={{ width: 36, height: 36, borderRadius: 18,
              backgroundColor: 'rgba(61,53,72,0.07)',
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}
          contentContainerStyle={{ paddingHorizontal: 16, gap: 7, paddingBottom: 12 }}>
          {/* All */}
          ...
          {/* Mood */}
          <Pressable onPress={() => setShowMoodPanel(p => !p)}
            style={[chipStyle, !!moodFilter && chipActiveStyle]}>
            {moodFilter
              ? <View style={{ width: 14, height: 14, borderRadius: 7, backgroundColor: PRO_MOODS[moodFilter].dot }} />
              : <Text style={{ fontSize: 13 }}>🎭</Text>}
            <Text style={[chipText, !!moodFilter && chipActiveText]}>
              {moodFilter ? PRO_MOODS[moodFilter].label : 'อารมณ์'}
            </Text>
            <Text style={{ fontSize: 9, color: C.inkFaint }}>{showMoodPanel ? '▲' : '▼'}</Text>
          </Pressable>
        </ScrollView>
        ...
        {showMoodPanel && (
          <View style={{ paddingHorizontal: 16, paddingBottom: 12,
            ...
            <Text style={{ fontSize: 11, fontWeight: '600', color: C.inkSoft,
              marginTop: 10, marginBottom: 8 }}>เลือกอารมณ์</Text>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {MOOD_KEYS.map(mk => {
                ...
                return (
                  <Pressable key={mk}
                    onPress={() => { setMoodFilter(active ? null : mk); setShowMoodPanel(false); }}
                    style={{ flex: 1, alignItems: 'center', paddingVertical: 8, borderRadius: 12,
                      backgroundColor: active ? m.dot + '33' : '#fff',
                      borderWidth: 1.5, borderColor: active ? m.dot : C.line }}>
                    <View style={{ width: 20, height: 20, borderRadius: 10,
                      backgroundColor: m.dot, marginBottom: 4 }} />
                    <Text style={{ fontSize: 9, color: C.inkSoft, fontWeight: active ? '700' : '500' }}>
                      {m.label}
                    </Text>
                  </Pressable>
                );
  ...
  return (
    <Pressable onPress={onPress}
      style={{
        ...
        shadowColor: C.ink, shadowOpacity: 0.05, shadowRadius: 8,
        shadowOffset: { width: 0, height: 2 }, elevation: 1,
      }}>

      {/* Mood accent bar */}
      {mood && (
        <View style={{ height: 3, backgroundColor: mood.dot }} />
      )}

      <View style={{ padding: 14 }}>
        {/* Top row: date + mood dot */}
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
          <Text style={{ fontSize: 11, color: C.inkFaint, flex: 1 }}>{dateStr}</Text>
          {mood && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: mood.dot }} />
        ...
        {/* Meta: photo count + expense count + location */}
            ...
            {media.slice(0, 4).map((m, i) => (
              <View key={m.id} style={{ position: 'relative' }}>
                <Image source={{ uri: m.localUri }}
                  style={{ width: 60, height: 60, borderRadius: 10, backgroundColor: C.line }} />
                {i === 3 && media.length > 4 && (
                  <View style={{
                    position: 'absolute', inset: 0, borderRadius: 10,
                    backgroundColor: 'rgba(61,53,72,0.55)',
                    justifyContent: 'center', alignItems: 'center',
                  }}>
...
const chipStyle = {
  ...
  backgroundColor: '#fff', borderWidth: 1.5, borderColor: '#E7DFD3',
};
const chipActiveStyle = {
  backgroundColor: '#FBEAF1', borderColor: '#D14F86',
};
const chipText = {
  fontSize: 12, fontWeight: '500' as const, color: '#7A7186',
};
